...
**Version:** 1.1  
**Status:** Decision Baseline After Product/Architecture Closure — Firebase-First Cost-Optimized Revision  
...
- MVP gameplay presentation runtime: Flame 2.5D (presentation-only).
- Authoritative rules remain Pure Dart (`game_engine` / `game_solver` / `level_generator`).
- Default presentation mode: Flame 2.5D.
- Flutter widget board remains a diagnostic fallback behind `FORCE_FLUTTER2D`.
- Unity is paused until a future full-3D phase; its code is not the current MVP path.
- Architecture reference: `docs/architecture/ADR_FLAME_2_5D_FIRST_v1.0.md`.
...
- Content blocker: `story_beats.json` still contains Layla/ليلى lines that conflict with Narrative Canon — do not build final cinematics from it until corrected.

---

# 6. APPROVED — Backend & Cloud

- MVP backend/cloud direction: **Firebase-first, serverless-first, cost-conscious**.
- Architectural principle: do not pay for always-on infrastructure unless a demonstrated product/scale requirement justifies it.
- Primary MVP cloud platform: Firebase / Google Cloud managed services.
- Player/cloud data: Cloud Firestore for suitable document-oriented state such as player profile, progression, Daily state, sync metadata, configuration references, and lightweight operational data.
- Content/assets: Firebase Storage for remote content bundles and illustration assets where appropriate.
...
- Game Engine, Solver, and Main Journey Level Generation remain primarily on-device to minimize latency and backend usage/cost.
...
- Environments:
  ...
  - STAGING.
  - PROD.
- Prefer Firebase/GCP managed scaling and quotas instead of pre-provisioned always-on compute.
- No Kubernetes in MVP.
- No Redis in MVP by default.
- No dedicated Message Broker in MVP by default.
- No always-on relational database in MVP by default.
- No Azure infrastructure is part of the MVP baseline.
- ASP.NET Core + PostgreSQL may be reconsidered later only if scale, CMS complexity, relational-query requirements, reporting, or operational constraints justify introducing them.
- Exact Firebase/GCP billing limits, quotas, Cloud Run/Functions resource settings, and DR targets remain operational tuning items.
...
- If a provider is already linked elsewhere, use an explicit conflict flow; no silent automatic merge.
- Active Attempt persistence: Local-first in Drift / SQLite.
- Durable cloud state includes progression/economy-related state appropriate for synchronization.
...
- Remote bundles/assets are hosted using Firebase Storage or another Firebase/GCP-native static delivery mechanism when justified.
...
- Backend/serverless observability:
  - Firebase / Google Cloud native logs and monitoring for Cloud Functions / Cloud Run.
  - no separate Azure observability stack.
- Ads: Google AdMob + Mediation.
- IAP client: Flutter `in_app_purchase`.
- IAP validation: server-side through trusted Cloud Functions / Cloud Run logic.
...
- Dedicated Redis/cache layer unless justified by measured load/use case.
- Dedicated Message Broker/queue platform unless justified by decoupling/scale.
- Kubernetes unless future scale/operational requirements justify it.
- ASP.NET Core + PostgreSQL backend unless Firebase-first architecture proves insufficient.
- Dedicated always-on backend infrastructure unless product scale or operational requirements justify it.

---

# 13A. SUPERSEDED — Replaced by Firebase-First MVP

The following previously approved Azure-heavy decisions are explicitly **superseded** for MVP and must not be implemented unless a future architecture review re-approves them:

- ASP.NET Core as mandatory MVP backend.
- Modular Monolith as mandatory MVP backend shape.
- PostgreSQL as mandatory MVP primary database.
- Azure as primary MVP cloud provider.
...
- Bicep as mandatory MVP IaC baseline.
- DB-backed background jobs as the default job model.
- Azure-specific sizing/SKU assumptions.

They are replaced by the approved Firebase-first/serverless-first baseline described in Sections 6–9.
...
At this point, the major product, gameplay, Firebase-first architecture, economy, progression, Daily, monetization, CMS, analytics, QA, security, and LiveOps baseline decisions required to update the existing project documents are substantially closed.

Remaining TBD items are mostly:

- commercial pricing.
- Firebase/GCP quota, billing and serverless resource tuning.
...
**End of Final Decision Register v1.1**
