...
final class DriftJourneyRepository implements JourneyRepository {
  ...
  Future<JourneyProgress> loadProgress() async {
    final row = await (_db.select(_db.journeyProgressRows)
          ..where((t) => t.id.equals(_mainId)))
        .getSingleOrNull();
    if (row == null) return const JourneyProgress();
    final completedLevels =
        (jsonDecode(row.completedLevelIdsJson) as List).cast<String>().toSet();
    final completedChapters =
        (jsonDecode(row.completedChapterIdsJson) as List).cast<String>().toSet();
  ...
  Future<void> saveProgress(JourneyProgress progress) async {
    await _db.into(_db.journeyProgressRows).insertOnConflictUpdate(
      db_lib.JourneyProgressRowsCompanion(
        ...
        completedLevelIdsJson:
            Value(jsonEncode(progress.completedLevelIds.toList())),
        completedChapterIdsJson:
            Value(jsonEncode(progress.completedChapterIds.toList())),
  ...
  Future<PlayerLocalFlags> loadFlags() async {
    final row = await (_db.select(_db.playerFlagRows)
          ..where((t) => t.id.equals(_mainId)))
        .getSingleOrNull();
    if (row == null) return const PlayerLocalFlags();
    final unlockedBeats =
        (jsonDecode(row.unlockedStoryBeatIdsJson) as List).cast<String>().toSet();
    final viewedBeats =
        (jsonDecode(row.viewedStoryBeatIdsJson) as List).cast<String>().toSet();
  ...
  Future<void> saveFlags(PlayerLocalFlags flags) async {
    await _db.into(_db.playerFlagRows).insertOnConflictUpdate(
      db_lib.PlayerFlagRowsCompanion(
        ...
        unlockedStoryBeatIdsJson:
            Value(jsonEncode(flags.unlockedStoryBeatIds.toList())),
        viewedStoryBeatIdsJson:
            Value(jsonEncode(flags.viewedStoryBeatIds.toList())),
        updatedAt: Value(DateTime.now().toUtc()),
      ),
    );
  }
