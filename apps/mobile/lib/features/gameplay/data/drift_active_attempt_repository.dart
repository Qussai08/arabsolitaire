...
final class DriftActiveAttemptRepository implements ActiveAttemptRepository {
  ...
  Future<SavedAttempt?> load() async {
    final row = await (_db.select(_db.activeAttempts)
          ..where((t) => t.id.equals(_activeId)))
        .getSingleOrNull();
  ...
  Future<void> save(SavedAttempt attempt) async {
    ...
    await _db.into(_db.activeAttempts).insertOnConflictUpdate(
  ...
  Future<void> clear() async {
    await (_db.delete(_db.activeAttempts)
          ..where((t) => t.id.equals(_activeId)))
        .go();
  }
}
