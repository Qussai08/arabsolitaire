...
void main() {
  group('config', () {
    test('rejects invalid association count / columns / moveLimit / capacity', () {
      expect(
        () => LevelConfiguration(
          levelDefinitionId: 'x',
  ...
  group('golden GG', () {
    ...
    test('GG-002 Stock Heavy template can accept', () {
      ...
      expect(level.difficultyMetrics.stockAdvancesInSolution, greaterThanOrEqualTo(0));
    }, timeout: const Timeout(Duration(minutes: 2)));
    ...
    test('GG-009 Solver Inconclusive handled cleanly', () {
      ...
      if (result is GenerationInconclusive) {
        expect(
          result.reason,
          GenerationFailureReason.solverInconclusiveBudget,
        );
      }
